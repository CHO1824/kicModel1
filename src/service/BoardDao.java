...
public class BoardDao {
	...
	public int nextNum() {
		...
		ResultSet rs = null;
		//column name을 입력하면 create table column순으로 입력한다.
		try {
	...
	public void refStepAdd(int ref, int refstep) {
		...
		//select num, ref, reflevel, refstep form board;
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
	...
	public int board(String boardid) {
		Connection con = JdbcConection.getConnection();
		PreparedStatement pstmt = null;
		String sql = "select count(*) from board where boardid = ?";
		ResultSet rs = null;
	...
	public List <Board> boardList(int pageInt, int limit, int boardcount, String boardid) {
		...
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, boardid);
			pstmt.setInt(2, (pageInt-1)*limit + 1); //4
